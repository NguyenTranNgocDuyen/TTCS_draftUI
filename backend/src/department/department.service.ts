...
import { BADREQUEST_CODE, CONFLIG_CODE, CREATED_RESPONE, nameRole_emloyee, nameRole_manager, NOTFOUND_CODE, OK_CODE } from 'src/common/code';
...
export class DepartmentService {
    ...
    async findAll(): Promise<ResponseDto<DepartmentDto[]>> {
        ...
        const departments: DepartmentDto[] | undefined = await this.prismaService.department.findMany();
        return {
            statusCode: OK_CODE,
    ...
    async createDepartment(DepartmentDto: CreateDepartmentDto): Promise<ResponseDto<DepartmentDto>> {
        ...
        try {
            ...
            const { statusCode, message, data } = await this.prismaService.$transaction(async (tx): Promise<ResponseDto<DepartmentDto>> => {
                ...
                if (managerID !== undefined) {
                    ...
                    if (managerRoleGet.roleID === managerGet.roleId) {
                        throw new ConflictException("This managerID is manager of another departmnet!!")
                    }
    ...
    async getDepartmentById(departmentID: string, tx? : Prisma.TransactionClient): Promise<ResponseDto<DepartmentDto>> {
    ...
    async getDepartmentByDeparmentName(departmentName: string): Promise<ResponseDto<DepartmentDto>> {


        const department: DepartmentDto | null = await this.prismaService.department.findUnique({
            where: {
                departmentName
            }
        })
        ...
        if (!department)
            return {
                statusCode: NOTFOUND_CODE, message: `The department has name = ${departmentName} is not found`
            }
    ...
    async updateDepartment(id: string, dto: UpdateDepartmentDto): Promise<ResponseDto<DepartmentDto>> {
        const { departmentName, managerID } = dto;

        try {
            return await this.prismaService.$transaction(async (tx) => {
                const currentDept = await tx.department.findUnique({ where: { departmentID: id } });
                ...
                if (departmentName && departmentName !== currentDept.departmentName) {
                    const nameCheck = await tx.department.findUnique({ where: { departmentName } });
                    if (nameCheck) throw new ConflictException("Department name already exists");
                }
    ...
    try {
     ...
     async handleManagerTransfer(
        ...
        if (currentManagerID && currentManagerID !== newManagerID) {
            const employeeRole = await tx.role.findUnique({ where: { nameRole: nameRole_emloyee } });
            await tx.user.update({
        ...
        if (newManagerID) {
            const newManager = await tx.user.findUnique({ where: { userID: newManagerID } });
            if (!newManager) throw new NotFoundException("New Manager not found");

            const managerRole = await tx.role.findUnique({ where: { nameRole: nameRole_manager } });

            if (newManager.roleId === managerRole.roleID && newManager.departmentID !== departmentID) {
                throw new ConflictException("This user is already managing another department");
            }
