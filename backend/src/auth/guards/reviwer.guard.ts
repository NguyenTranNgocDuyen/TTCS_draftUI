...
import { BADREQUEST_CODE, OK_CODE } from "src/common/code";
...
export default class ReviewAuthGuards implements CanActivate{
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (monthlyTimesheetID === undefined)
            throw new UnauthorizedException("The monthlyTimesheetID cannot undefined")
        ...
        for (const permission of allowedPermission) {
            ...
            if (permission === 'manager'){
                ...
                if (monthlyTimesheet.isSubmitted === false)
                    throw new BadRequestException('This monthly timesheet was not submitted')
