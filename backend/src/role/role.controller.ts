...
export class RoleController {
  ...
  async create(@Body() createRoleDto: CreateRoleDto): Promise<ResponseDto<RoleDto> | AnotherError> {
    console.log('Inide role.controller.create()')
    const { statusCode, message, data }: ResponseDto<RoleDto> = await this.roleService.create(createRoleDto);
  ...
  async findAll(): Promise<ResponseDto<RoleDto[]> | AnotherError> {
    const { statusCode, message, data }: ResponseDto<RoleDto[]> = await this.roleService.findAll(); // Đổi users -> roles
  ...
  @RequirePermission('admin')
  @UseGuards(JwtAuthGuard, UserAccessGaurd)
  async findOne(@Param('id', new ParseUUIDPipe()) id: string): Promise<ResponseDto<RoleDto> | AnotherError> {

    const { statusCode, message, data }: ResponseDto<RoleDto> = await this.roleService.findOne(id);
  ...
  @RequirePermission('admin')
  @UseGuards(JwtAuthGuard, UserAccessGaurd)
  async update(@Param('id', new ParseUUIDPipe()) id: string, @Body() updateRoleDto: UpdateRoleDto): Promise<ResponseDto<RoleDto> | AnotherError> {

    const { statusCode, message, data }: ResponseDto<RoleDto> | AnotherError = await this.roleService.update(id, updateRoleDto);
    if (statusCode === CONFLIG_CODE)
      throw new ConflictException(statusCode, message)
    if (statusCode == NOTFOUND_CODE)
  ...
  @RequirePermission('admin')
  @UseGuards(JwtAuthGuard, UserAccessGaurd)
  async remove(@Param('id', new ParseUUIDPipe()) id: string): Promise<ResponseDto<RoleDto> | AnotherError> {
    const { statusCode, message, data }: ResponseDto<RoleDto> = await this.roleService.remove(id);
    if (statusCode === NOTFOUND_CODE)
      throw new NotFoundException(statusCode, message)
    if (statusCode === OK_CODE)
      return {
  ...
  async getRoleBYNameRole(@Param("nameRole") nameRole: string): Promise<ResponseDto<RoleDto> | AnotherError> {
    const { statusCode, message, data }: ResponseDto<RoleDto> = await this.roleService.getRoleByRoleName(nameRole)
    if (statusCode === NOTFOUND_CODE)
